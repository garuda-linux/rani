...
const sources = ['games', 'documents', 'internet', 'multimedia', 'other', 'science', 'security', 'multimedia'];

for (const source of sources) {
  if (!existsSync(source)) continue;
  const entries = readFileSync(`./sources/${source}.txt`, 'utf8').split('\n');
  ...
  writeFileSync(`./parsed/${source}-repo.json`, JSON.stringify(toDisplay.sort((a, b) => a.name.localeCompare(b.name))));
  writeFileSync(`./parsed/${source}-aur.json`, JSON.stringify(inAur.sort((a, b) => a.name.localeCompare(b.name))));
}
