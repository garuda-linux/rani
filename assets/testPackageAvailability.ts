...
import { gamingPackageLists } from "../assets/package-lists/gaming";
import { readFileSync } from "node:fs";
import { FullPackageDefinition } from "@app/renderer/src/app/components/gaming/interfaces";

const inRepo: string[] = execSync("pacman -Ssq").toString().split("\n");
const inAur: string[] = execSync("paru -Slaq", { maxBuffer: 10240000 })
  .toString()
  .split("\n");

console.log(
  `In total ${inRepo.length} packages were found in the repositories`,
);
...
function checkGaming(): number {
  console.log(
    "Checking package availability of gaming packages in repositories",
  );
...
function checkGeneral(): number {
  ...
  for (const section of toCheck) {
    ...
    const packages: FullPackageDefinition[] = JSON.parse(
    ...
    for (const pkg of packages) {
      if (!inRepo.includes(pkg.pkgname[0]) && !inAur.includes(pkg.pkgname[0])) {
        ...
        !inRepo.includes(pkg.pkgname[0]) &&
        inAur.includes(pkg.pkgname[0])
      ) {
