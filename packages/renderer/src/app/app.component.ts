...
export class AppComponent implements OnInit {
  ...
  constructor() {
    effect(() => {
      ...
      const index: number = curItems.findIndex(
        (item) => item['translocoKey'] === 'menu.terminal',
      );
      if (index === -1) return;
  ...
  private attachElectronListeners() {
    // Note: Window resize events would need to be handled differently in Electron
    // This could be implemented through IPC events if needed
