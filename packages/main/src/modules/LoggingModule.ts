...
class LoggingModule implements AppModule {
  ...
  private setupLoggingHandlers(): void {
    ...
    ipcMain.handle('log:structured', async (_, level: string, message: string, data?: unknown) => {
      try {
    ...
    ipcMain.handle('log:withContext', async (_, level: string, message: string, context: Record<string, unknown>) => {
      try {
        const timestamp = new Date().toISOString();
