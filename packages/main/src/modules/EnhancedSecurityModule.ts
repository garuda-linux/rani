    ...
    app.on('web-contents-created', (event, contents) => {
      ...
      contents.on('will-navigate', (event, navigationUrl) => {
        try {
          ...
          console.warn(`Invalid navigation URL blocked: ${navigationUrl}`, error);
          event.preventDefault();
        }
      });
      ...
      contents.on('will-navigate', (event, navigationUrl) => {
        try {
          ...
          console.warn(`Invalid frame navigation URL blocked: ${navigationUrl}`, error);
          event.preventDefault();
        }
      });
      ...
      contents.on('console-message', (event, level, message, line, sourceId) => {
        ...
        if (securityKeywords.some((keyword) => message.toLowerCase().includes(keyword))) {
          console.warn(`Potential security issue detected in renderer: ${message}`);
        }
      });
    });
  }
