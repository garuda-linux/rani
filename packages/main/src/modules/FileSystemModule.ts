  ...
  private setupFileSystemHandlers(): void {
    ...
    ipcMain.handle('fs:readTextFile', async (_, filePath: string) => {
      try {
        ...
        this.logger.error(`File read error: ${error instanceof Error ? error.message : String(error)}`);
    ...
    ipcMain.handle('fs:writeTextFile', async (_, filePath: string, contents: string) => {
      try {
        if (!validateFilePath(filePath)) {
          throw new Error('Invalid file path');
        }
        ...
        this.logger.error(`File write error: ${error instanceof Error ? error.message : String(error)}`);
    ...
    ipcMain.handle('fs:createDirectory', async (_, dirPath: string) => {
      try {
        ...
        this.logger.error(`Directory creation error: ${error instanceof Error ? error.message : String(error)}`);
        throw new Error(`Failed to create directory: ${error instanceof Error ? error.message : error}`);
      }
    });
    ...
    ipcMain.handle('fs:removeFile', async (_, filePath: string) => {
      try {
        ...
        this.logger.error(`File removal error: ${error instanceof Error ? error.message : String(error)}`);
        throw new Error(`Failed to remove file: ${error instanceof Error ? error.message : error}`);
      }
    });
  }
}
