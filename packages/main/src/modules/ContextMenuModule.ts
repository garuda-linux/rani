...
class ContextMenuModule implements AppModule {
  ...
  private setupContextMenuHandlers(): void {
    ...
    ipcMain.handle('contextMenu:show', async (_, items: ContextMenuItem[], x?: number, y?: number) => {
      console.log('ContextMenuModule:show', items, x, y);
      try {
        ...
        console.error('Context menu show error:', error);
        throw new Error(`Failed to show context menu: ${error instanceof Error ? error.message : error}`);
      }
    });
  }
