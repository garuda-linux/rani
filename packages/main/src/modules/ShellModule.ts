...
class ShellModule implements AppModule {
  ...
  constructor(isDevelopment = false) {
    ...
    if (this.isDevelopment) {
      try {
        ...
        console.warn('SSH key not found, SSH functionality will be disabled:', error);
      }
    }
  }
  ...
  private setupShellHandlers(): void {
    ...
    const validateCommand = (command: string, args: string[]): boolean => {
      ...
      if (this.isDevelopment) {
        console.log(`[DEV MODE] Allowing command: ${command} ${args.join(' ')}`);
        return true;
      }
  ...
  private setupStreamingHandlers(): void {
    ...
    ipcMain.on('shell:stdout', (event, data) => {
      console.log('[MAIN] Received shell:stdout:', data.processId, data.data?.substring(0, 100));
      try {
    ...
    ipcMain.on('shell:stderr', (event, data) => {
      console.log('[MAIN] Received shell:stderr:', data.processId, data.data?.substring(0, 100));
      try {
    ...
    ipcMain.on('shell:close', (event, data) => {
      console.log('[MAIN] Received shell:close:', data.processId, 'code:', data.code);
      try {
    ...
    ipcMain.on('shell:error', (event, data) => {
      console.log('[MAIN] Received shell:error:', data.processId, data.error?.message);
      try {
