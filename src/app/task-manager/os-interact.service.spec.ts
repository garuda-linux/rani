...
describe('OsInteractService', () => {
  ...
  it('getServices should parse JSON and active state', async () => {
    const mockJson = JSON.stringify([
      { unit: 'active.service', active: 'active', load: 'loaded', sub: 'running' },
      { unit: 'inactive.service', active: 'inactive', load: 'loaded', sub: 'dead' },
      { unit: 'active-socket.socket', active: 'active', load: 'loaded', sub: 'listening' },
    ]);
  ...
  it('generateTasks should create install/uninstall tasks based on wantedPackages', async () => {
    await runInInjectionContext(injector, async () => {
      ...
      const dummyTask = { id: 'dummy', name: 'dummy' /* other required Task props */ } as Task;
