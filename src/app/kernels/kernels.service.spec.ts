...
describe('KernelsService', () => {
  ...
  it('getDkmsStatus should parse valid output and set dkmsModules signal', async () => {
    ...
    expect(service.dkmsModules()).toEqual([
      { moduleName: 'acpi_call', moduleVersion: '1.2.2', kernelVersion: '6.1.1-a', status: 'installed' },
      { moduleName: 'vboxhost', moduleVersion: '7.0', kernelVersion: '6.1.1-a', status: 'installed' },
    ]);
  ...
  it('getDkmsStatus should handle broken status correctly', async () => {
    ...
    expect(service.dkmsModules()).toEqual([
      { moduleName: 'nvidia', moduleVersion: '550.1', kernelVersion: '6.2.2-z', status: 'broken' },
    ]);
  ...
  it('dkmsModulesBroken computed signal should be true if any module status is not "installed"', async () => {
    await runInInjectionContext(injector, async () => {
      service.dkmsModules.set([
        { moduleName: 'nvidia', moduleVersion: '550', kernelVersion: '6.2.2-zen', status: 'installed' },
        { moduleName: 'vboxhost', moduleVersion: '7.0', kernelVersion: '6.1.1-arch', status: 'broken' },
      ]);
  ...
  it('updateKernelStatus should identify missing DKMS modules for installed kernels (linux)', () => {
    runInInjectionContext(injector, () => {
      ...
      service.dkmsModules.set([
        { moduleName: 'nvidia', moduleVersion: '550', kernelVersion: '6.14.0-4-arch1-1', status: 'installed' },
      ]);
