...
export class PrivatebinClient extends Api {
  ...
  private async getPaste(id: string): Promise<PrivatebinPasteRequest> {
    const response = await this.get<PrivatebinPasteRequest, AxiosResponse<PrivatebinPasteRequest>>(`/?pasteid=${id}`);
  ...
  private async postPaste(
    ...
    const response = await this.post<PrivatebinResponse, PrivatebinPasteRequest, AxiosResponse<PrivatebinResponse>>(
      '/',
      {
        v: 2,
        ct,
